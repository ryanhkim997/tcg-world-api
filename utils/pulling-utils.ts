...
export function getHmacSeed(serverSeed: string, clientSeed: string, nonce: number): string {
  ...
  export function weightedChoice(rng: () => number, items: Card[], k = 1): string[] {
    const values = items.map((c) => c.name);
    const weights = items.map((c) => c.weight);
    const cumWeights = weights.map((sum => (value => sum += value))(0));
